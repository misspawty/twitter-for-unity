...
//				xy[1] = xy[1] + 10000000.0;
		...
		public Vector2 ConvertToLatLong(float _X, float _Y, double _zone, Hemisphere _Hemi){
			float offsetX = 707160.9f;
			float offsetY = 5714473f;

			ToLatLon ((double)(_X + offsetX), (double)(_Y + offsetY), (int)_zone, _Hemi);
			return new Vector2 ((float)Latitude, (float)Longitude);
		}

		public Vector3 ConvertToUnityCoords(float _Latitude, float _Longitude){
			float offsetX = 707160.9f;
			float offsetY = 5714473f;

			ToUTM ((double)_Latitude, (double)_Longitude);

			return new Vector3 ((float)X - offsetX, 0f, (float)Y - offsetY);
		}
